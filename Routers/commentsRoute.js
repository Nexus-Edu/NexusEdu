...
router.post('/makeComment', async (req, res) => {
    const data = req.body;
    try {
        const response = await pool.query(`INSERT INTO comments (post_id, user_id, comment) values ($1, $2, $3) returning *;`, [data.user_id, data.post_id, data.comment]) 
